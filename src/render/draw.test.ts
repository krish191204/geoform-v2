...
describe('draw', () => {
  ...
  it('lights relief from the northwest and raises contrast toward crests', () => {
    const ridge = makeWorld({
      width: 7,
      height: 3,
      elev: (x) => (x === 3 ? 2800 : x === 2 || x === 4 ? 900 : 180),
      moistMean: () => 0.55,
      summerMoist: () => 0.2,
      summer: () => 10,
    })
    const img = draw(ridge, 'summer', 'relief')
    const luma = (x: number) => {
      const p = pixel(img, x, 1)
      return p[0] + p[1] + p[2]
    }
    // West face catches upper-left light; the east face sits in soft shadow.
    expect(luma(2)).toBeGreaterThan(luma(4) + 8)

    const gap = (base: number) => {
      const world = makeWorld({
        width: 5,
        height: 3,
        elev: (x) => (x === 2 ? base + 700 : base),
        moistMean: () => 0.5,
        summerMoist: () => 0.2,
        summer: () => 10,
      })
      const painted = draw(world, 'summer', 'relief')
      const sum = (x: number) => {
        const p = pixel(painted, x, 1)
        return p[0] + p[1] + p[2]
      }
      return Math.abs(sum(1) - sum(3))
    }
    expect(gap(3200)).toBeGreaterThan(gap(120) + 6)
  })

  it('blends humid lowlands green, dry lowlands brown, and pales the highlands', () => {
    const paint = (elev: number, moist: number) =>
      pixel(
        draw(
          makeWorld({
            width: 4,
    ...
    const humid = paint(220, 0.92)
    const dry = paint(220, 0.05)
    const high = paint(4600, 0.92)
    expect(humid[1]).toBeGreaterThan(humid[0])
    expect(dry[0]).toBeGreaterThan(dry[2])
    expect(dry[0]).toBeGreaterThan(humid[0])
    expect(high[0] + high[1] + high[2]).toBeGreaterThan(humid[0] + humid[1] + humid[2])
  })

  it('paints the shelf lighter than open ocean and keeps foam off the deep water', () => {
    const world = makeWorld({ width: 16, height: 4, elev: () => 0, summer: () => 16 })
    world.mask.fill(0)
    world.mask[1 * 16 + 0] = 1
    world.elev[1 * 16 + 0] = 180
    const flat = draw(world, 'summer', 'relief')
    const sum = (p: [number, number, number]) => p[0] + p[1] + p[2]
    const shelf = pixel(flat, 2, 1)
    const open = pixel(flat, 12, 1)
    expect(sum(shelf)).toBeGreaterThan(sum(open) + 20)
    expect(sum(open)).toBeLessThan(220)

    const zoom = draw(world, 'summer', 'relief', { scale: 8, smooth: true })
    const shore = pixel(zoom, 8, 16)
    const deep = pixel(zoom, 48, 16)
    expect(shore[0]).toBeLessThan(180)
    expect(sum(deep)).toBeLessThan(sum(shelf) + 40)
  })
})
